class Stream < ActiveRecord::Base
  ...
  def get_qs_counter
    ...
    self.requests.sort_by{|r| r.start_date }.each { |r|
      if ((WORKPACKAGE_QS.include?(r.work_package[0..6]) or WORKPACKAGE_QS.include?(r.work_package[0..11])) and (r.counter_log) and (r.counter_log.validity))
  ...
  def get_spider_counter
    total = 0
    self.requests.sort_by{|r| r.start_date }.each { |r|
      if ((WORKPACKAGE_SPIDERS.include?(r.work_package[0..6]) or WORKPACKAGE_SPIDERS.include?(r.work_package[0..11])) and (r.counter_log) and (r.counter_log.validity))
         total = total + r.counter_log.counter_value
      end
    }
    return total
  ...
  def get_current_spider_counter_request(author)
    ...
    self.spider_requests_sorted_by_counters.each { |r|
      if ((WORKPACKAGE_SPIDERS.include?(r.work_package[0..6]) or WORKPACKAGE_SPIDERS.include?(r.work_package[0..11])) and (r.counter_log) and (r.counter_log.validity) and (r.assigned_to == author.rmt_user))
  ...
  def get_current_qs_counter_request(author)
    ...
    self.qs_requests_sorted_by_counters.each { |r|
       if ((WORKPACKAGE_QS.include?(r.work_package[0..6]) or WORKPACKAGE_QS.include?(r.work_package[0..11])) and (r.counter_log) and (r.counter_log.validity) and (r.assigned_to == author.rmt_user))
