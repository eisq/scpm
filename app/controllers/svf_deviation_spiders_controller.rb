require 'spreadsheet'
class SvfDeviationSpidersController < ApplicationController
	layout "spider"
	...
	def index
	    ...
	    if @meta_activity_id == nil
	    	@meta_activity = SvfDeviationMetaActivity.find(:first)
	    	@meta_activity_id = @meta_activity.id
	    else
	    	@meta_activity = SvfDeviationMetaActivity.find(:first, :conditions=>["id = ?", @meta_activity_id])
	    end
	    @meta_activities = SvfDeviationMetaActivity.find(:all, :conditions=>["is_active = ?", true]).map { |ma| [ma.name, ma.id] }
	    ...
	    if milestone_id
		    ...
		    @show_bilan_custo = get_show_bilan_custo(@milestone)
	   		...
	   		@last_spider = SvfDeviationSpider.last(:conditions => ["milestone_id= ?", milestone_id])
	   		if @last_spider
	    	...
	    	if (@last_spider)
	    		if @last_spider.svf_deviation_spider_consolidations.count == 0
			    	generate_current_table(@last_spider, @meta_activity_id)
			    	check_meta_activities(@last_spider.id, @meta_activities)

			    	conso_temp = SvfDeviationSpiderConsolidationTemp.find(:first, :conditions=>["svf_deviation_spider_id = ?", @last_spider.id])
			    	if !conso_temp
				    	deviation_spider_temp_init = SvfDeviationSpiderConsolidationTemp.new
				    	deviation_spider_temp_init.svf_deviation_spider_id = @last_spider.id
	...
	def get_last_import_date(project_id)
		var = nil
		var = SvfDeviationSpiderReference.find(:first, :conditions=>["project_id = ?", project_id], :order=>"version_number desc")
		if var
			var = var.created_at
		end
	...
	def index_history
	    ...
	    @meta_activities = SvfDeviationMetaActivity.all.map { |ma| [ma.name, ma.id] }

	    if deviation_spider_id
	   		@last_spider = SvfDeviationSpider.find(:first, :conditions => ["id = ?", deviation_spider_id])
	...
	def update_spider_file_name_form
	    spider_id  = params[:id]
	    @spider    = SvfDeviationSpider.find(spider_id)
  	end

  	def update_spider_file_name
	    spider_id          = params[:id]
	    if params[:svf_deviation_spider][:file_link]
	      spider           = SvfDeviationSpider.find(spider_id)
	      spider.file_link = params[:svf_deviation_spider][:file_link]
	      spider.save
	    end
	...
	def export_customization_excel
		...
		if @project
			begin
				...
				deviation_spider_reference_last = SvfDeviationSpiderReference.find(:last, :conditions => ["project_id = ?", project_id], :order => "version_number")
				@exportCustomizations = Array.new
				# DeliverablesSettings: for each deliverable setting
				deviationSpiderSettings = SvfDeviationSpiderSetting.find(:all, :conditions => ["svf_deviation_spider_reference_id = ?", deviation_spider_reference_last]).each do |devia_settings|
	...
	def consolidate_interface
	    deviation_spider_id = params[:deviation_spider_id]
	    if deviation_spider_id
	    	@deviation_spider 	= SvfDeviationSpider.find(:first, :conditions => ["id = ?", deviation_spider_id])

	    	redirect = false
		    SvfDeviationSpiderDeliverable.find(:all, :conditions=>["svf_deviation_spider_id = ?", deviation_spider_id]).each do |deliverable|
		    	SvfDeviationSpiderValue.find(:all, :conditions=>["svf_deviation_spider_deliverable_id = ?", deliverable]).each do |value|
		    		if value.answer == nil
		    			redirect = true
		    		end
		    	end
		    ...
		    if redirect
		    	redirect_to('/svf_deviation_spiders?milestone_id='+@deviation_spider.milestone_id.to_s+'&empty=1')
			end
			...
			#get all svf deviation spiders linked to this project
			deviation_spiders = Array.new
			deviation_spiders = get_svf_deviation_spiders(@deviation_spider)
	...
	def get_activities_realisation(maturity_deliverables)
		activities_partially_realised = Array.new
		activities_not_realised = Array.new
		activity_realisations = Array.new
		...
		all_activities.each { |activity|
			...
			deliverables.each do |deliv|
				SvfDeviationMacroActivity.find(:all, :conditions=>["svf_deviation_activity_id = ?", activity_struct.activity.id]).each do |macro_activity|
					macro_activity_deliverable = SvfDeviationMacroActivityDeliverable.find(:first, :conditions=>["svf_deviation_macro_activity_id = ? and svf_deviation_deliverable_id = ?", macro_activity.id, deliv.id])
	...
	def get_number_template(deviation_spider)
		...
		SvfDeviationSpiderMaturity.find(:all, :conditions=>["svf_deviation_spider_id = ?", deviation_spider.id]).each do |maturity|
	...
	def get_svf_deviation_spiders(deviation_spider)
		spiders = Array.new
		SvfDeviationSpider.find(:all, :conditions=>["project_id = ?", deviation_spider.project_id], :order=>"id asc").each do |devia_spider|
			spiders.each do |sp|
	...
	def get_deliverable_activity_applicable(project_id, deliverable, activity, milestone, psu_imported=true)
		applicable = false
		if psu_imported
			last_reference = SvfDeviationSpiderReference.find(:last, :conditions => ["project_id = ?", project_id], :order => "version_number asc")
			SvfDeviationSpiderSetting.find(:all, :conditions=>["svf_deviation_spider_reference_id = ? and deliverable_name = ? and activity_name = ?", last_reference, deliverable.name, activity.name]).each do |setting|
	...
	def get_deliverable_is_well_used(deviation_spider_id, deliverable, activity)
		well_used = false
		deviation_deliverable = SvfDeviationSpiderDeliverable.find(:first, :conditions=>["svf_deviation_spider_id = ? and svf_deviation_deliverable_id = ? and not_done = ?", deviation_spider_id, deliverable.id, 0])
		if deviation_deliverable
			SvfDeviationQuestion.find(:all, :conditions=>["svf_deviation_deliverable_id = ? and svf_deviation_activity_id = ?", deliverable.id, activity.id]).each do |question|
				deviation_value = SvfDeviationSpiderValue.find(:all, :conditions=>["svf_deviation_spider_deliverable_id = ? and svf_deviation_question_id = ?", deviation_deliverable.id, question.id]).each do |value|
					if value.answer
						well_used = true
					end
				end
	...
	def existing_question_activity_deliverable(deliverable_id, activity_id, milestone)
		...
		SvfDeviationQuestion.find(:all, :conditions => ["svf_deviation_deliverable_id = ? and svf_deviation_activity_id = ? and is_active = ?", deliverable_id, activity_id, true]).each do |question|
			question_milestone = SvfDeviationQuestionMilestoneName.find(:first, :conditions=>["svf_deviation_question_id = ? and milestone_name_id = ?", question.id, milestone_name_id])
	...
	def update_score
		svf_deviation_spider_maturity_id 		= params[:svf_deviation_spider_maturity_id]
		svf_deviation_spider_maturity_achieved 	= params[:svf_deviation_spider_maturity_achieved]
		if svf_deviation_spider_maturity_id and svf_deviation_spider_maturity_achieved
			svf_deviation_spider_maturity = SvfDeviationSpiderMaturity.find(:first, :conditions => ["id = ?", svf_deviation_spider_maturity_id])
			svf_deviation_spider_maturity.achieved = svf_deviation_spider_maturity_achieved
			svf_deviation_spider_maturity.save
		end
	...
	def update_planned
		svf_deviation_spider_maturity_id 		= params[:svf_deviation_spider_maturity_id]
		svf_deviation_spider_maturity_planned 	= params[:svf_deviation_spider_maturity_planned]
		if svf_deviation_spider_maturity_id and svf_deviation_spider_maturity_planned
			svf_deviation_spider_maturity = SvfDeviationSpiderMaturity.find(:first, :conditions => ["id = ?", svf_deviation_spider_maturity_id])
			svf_deviation_spider_maturity.planned = svf_deviation_spider_maturity_planned
			svf_deviation_spider_maturity.save
		end
		render(:nothing=>true)
	end
	...
	def update_comment
		...
		if deviation_spider_maturity_id and deviation_spider_maturity_comment
			deviation_spider_maturity = SvfDeviationSpiderMaturity.find(:first, :conditions => ["id = ?", deviation_spider_maturity_id])
	...
	def consolidate
		...
		if deviation_spider_id
		    ...
		    deviation_spider_consolidation = SvfDeviationSpiderConsolidation.new
	    	deviation_spider_consolidation.svf_deviation_spider_id = deviation_spider_id
	    	deviation_spider_consolidation.save

	    	# Delete consolidation temps
	    	SvfDeviationSpiderConsolidationTemp.find(:all, :conditions=>["svf_deviation_spider_id = ?", deviation_spider_id]).each do |conso_temp|
	    		conso_temp.delete
	    	end
	...
	def delete_current_spider
    	...
    	if deviation_spider.svf_deviation_spider_consolidations.count == 0
    		SvfDeviationSpiderActivityValue.find(:all, :conditions=>["svf_deviation_spider_id = ?", deviation_spider.id]).each do |activityvalue|
    			activityvalue.delete
    		end
    		SvfDeviationSpiderDeliverableValue.find(:all, :conditions=>["svf_deviation_spider_id = ?", deviation_spider.id]).each do |deliverablevalue|
    			deliverablevalue.delete
    		end
    		SvfDeviationSpiderDeliverable.find(:all, :conditions=>["svf_deviation_spider_id = ?", deviation_spider.id]).each do |deliverable|
    			SvfDeviationSpiderValue.find(:all, :conditions=>["svf_deviation_spider_deliverable_id = ?", deliverable.id]).each do |value|
    				value.delete
    			end
    			deliverable.delete
    		end
	...
	def delete_consolidated_spider
	    deviation_spider_id = params[:deviation_spider_id]
    	deviation_spider 	= SvfDeviationSpider.find(:first, :conditions=>["id = ?", deviation_spider_id])
    	if deviation_spider and deviation_spider.svf_deviation_spider_consolidations.count > 0
    		SvfDeviationSpiderActivityValue.find(:all, :conditions=>["svf_deviation_spider_id = ?", deviation_spider.id]).each do |activityvalue|
    			activityvalue.delete
    		end
    		SvfDeviationSpiderConsolidation.find(:all, :conditions=>["svf_deviation_spider_id = ?", deviation_spider.id]).each do |consolidation|
	...
	def create_spider
		...
		if @milestone
			@deviation_spider = SvfDeviationSpider.new
	...
	def update_question
		...
		deviation_spider_value = SvfDeviationSpiderValue.find(:first, :conditions => ["id = ?", deviation_spider_value_id])
		...
		if to_boolean(deviation_spider_value_answer)
			deviation_spider_value.svf_deviation_spider_deliverable.not_done = false
			deviation_spider_value.svf_deviation_spider_deliverable.save
		end
		...
		deviation_spider = deviation_spider_value.svf_deviation_spider_deliverable.svf_deviation_spider
		
		deviation_spider.svf_deviation_spider_deliverables.each do |deliverable|
			deliverable.svf_deviation_spider_values.each do |value|
				if deviation_spider_value.svf_deviation_spider_deliverable_id == value.svf_deviation_spider_deliverable_id and deviation_spider_value.svf_deviation_question.question_text == value.svf_deviation_question.question_text and value.id != deviation_spider_value.id
	...
	def update_file_link
		...
		deviation_spider 	= SvfDeviationSpider.find(:last, :conditions=>["id = ?", deviation_spider_id], :order=>"id")
	...
	def delete_spider_deliverable
		...
		if deviation_spider_deliverable_id
			deviation_spider_deliverable = SvfDeviationSpiderDeliverable.find(:first, :conditions => ["id = ?", deviation_spider_deliverable_id])
			milestone_id = deviation_spider_deliverable.svf_deviation_spider.milestone_id
	...
	def add_spider_deliverable
		...
		if deviation_spider_id and deviation_deliverable_id
			deviation_deliverable 		= SvfDeviationDeliverable.find(:first, :conditions => ["id = ?", deviation_deliverable_id])
			deviation_spider 			= SvfDeviationSpider.find(:first, :conditions => ["id = ?", deviation_spider_id])
	...
	def set_false_for_spider_deliverable
		...
		if deviation_spider_deliverable_id
			...
			deviation_spider_deliverable.svf_deviation_spider_values.each do |s|
				s.answer = 0
				s.save
			end

			redirect_to :action=>:index, :milestone_id=>deviation_spider_deliverable.svf_deviation_spider.milestone_id, :meta_activity_id=>meta_activity_id
		else
			redirect_to :controller=>:projects, :action=>:index
		end
	end
	...
	def generate_current_table(spider, meta_activity_id)
		@questions = SvfDeviationSpiderValue.find(:all, 
		:joins => ["JOIN svf_deviation_spider_deliverables ON svf_deviation_spider_deliverables.id = svf_deviation_spider_values.svf_deviation_spider_deliverable_id",
		"JOIN svf_deviation_questions ON svf_deviation_questions.id = svf_deviation_spider_values.svf_deviation_question_id",
		"JOIN svf_deviation_activities ON svf_deviation_activities.id = svf_deviation_questions.svf_deviation_activity_id",
		"JOIN svf_deviation_deliverables ON svf_deviation_deliverables.id = svf_deviation_spider_deliverables.svf_deviation_deliverable_id"], 
		:conditions => ["svf_deviation_spider_deliverables.svf_deviation_spider_id = ? and svf_deviation_activities.svf_deviation_meta_activity_id = ? and svf_deviation_deliverables.is_active = ? and svf_deviation_activities.is_active = ? and svf_deviation_questions.is_active = ?", spider.id, meta_activity_id, true, true, true], 
		:order => "svf_deviation_activities.name , svf_deviation_deliverables.name, svf_deviation_questions.question_text")

		#Search in the list of all deliverables for a milestone, if there is one which is not present in the current spider.
		last_reference 			= SvfDeviationSpiderReference.find(:last, :conditions => ["project_id = ?", spider.milestone.project_id], :order => "version_number asc")
		deliverable_ids 		= spider.svf_deviation_spider_deliverables.map {|d| d.svf_deviation_deliverable_id }
		...
		deliverables_to_add = SvfDeviationDeliverable.find(:all, 
		                      :joins=>["JOIN svf_deviation_questions ON svf_deviation_questions.svf_deviation_deliverable_id = svf_deviation_deliverables.id", 
		                      	"JOIN svf_deviation_question_milestone_names ON svf_deviation_question_milestone_names.svf_deviation_question_id = svf_deviation_questions.id",
		                      	"JOIN milestone_names ON milestone_names.id = svf_deviation_question_milestone_names.milestone_name_id",
		                      	"JOIN svf_deviation_question_lifecycles ON svf_deviation_question_lifecycles.svf_deviation_question_id = svf_deviation_questions.id"], 
		                      	:conditions => ["svf_deviation_deliverables.id NOT IN (?) and svf_deviation_question_lifecycles.lifecycle_id = ? and milestone_names.title = ? and svf_deviation_deliverables.is_active = ?", deliverable_ids_cleaned, spider.milestone.project.lifecycle_object.id, spider.milestone.name, true]).map { |d| [d.name, d.id]}
	...
	def supposed_to_be_added(spider_id, last_reference_id, deliverable_id)
		to_add 				= false
		deliverable 		= SvfDeviationDeliverable.find(:first, :conditions => ["id = ? and is_active = ?", deliverable_id, true])
		if deliverable
			spider_deliverable 	= SvfDeviationSpiderDeliverable.find(:first, :conditions => ["svf_deviation_spider_id = ? and svf_deviation_deliverable_id = ?", spider_id, deliverable_id])
			settings 			= SvfDeviationSpiderSetting.find(:all, :conditions => ["svf_deviation_spider_reference_id = ? and deliverable_name = ?", last_reference_id, deliverable.name])
			if settings
	...
	def check_meta_activities(spider_id, meta_activities)
		...
		meta_activities.each do |meta_activity|
			questions_nil_count = SvfDeviationSpiderValue.count(
			    :joins => ["JOIN svf_deviation_spider_deliverables ON svf_deviation_spider_deliverables.id = svf_deviation_spider_values.svf_deviation_spider_deliverable_id",
				"JOIN svf_deviation_questions ON svf_deviation_questions.id = svf_deviation_spider_values.svf_deviation_question_id",
				"JOIN svf_deviation_activities ON svf_deviation_activities.id = svf_deviation_questions.svf_deviation_activity_id"],
				:conditions => ["svf_deviation_spider_values.answer IS NULL and svf_deviation_activities.svf_deviation_meta_activity_id = ? and svf_deviation_spider_deliverables.svf_deviation_spider_id = ?", meta_activity[1], spider_id])
	...
	def generate_spider_history(milestone)
		@history = Array.new
		SvfDeviationSpider.find(:all,
		    :select => "DISTINCT(svf_deviation_spiders.id),svf_deviation_spiders.created_at, svf_deviation_spiders.impact_count",
    		:joins => 'JOIN svf_deviation_spider_consolidations ON svf_deviation_spiders.id = svf_deviation_spider_consolidations.svf_deviation_spider_id',
