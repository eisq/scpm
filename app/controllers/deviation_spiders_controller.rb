...
class DeviationSpidersController < ApplicationController
	...
	def get_deviation_status(deviation_spider, deliverable, activity, score)
		...
		SvtDeviationSpiderSetting.find(:all, :conditions=>["deviation_spider_reference_id = ? and deliverable_name = ? and activity_name = ?", last_reference, deliverable.name, activity.name]).each do |setting|
	...
	def generate_current_table(spider, meta_activity_id)
		...
		:conditions => ["deviation_spider_deliverables.deviation_spider_id = ? and deviation_activities.deviation_meta_activity_id = ? and deviation_deliverables.is_active = ? and deviation_activities.is_active = ? and deviation_questions.is_active = ?", spider.id, meta_activity_id, true, true, true],
