  ...
  def show_informations_by_qr_qwr
    ...
    Person.find(:all,:include => [:person_roles,:roles], :conditions=>["roles.name = 'QR'"], :order=>"people.name asc").each do |qr|    
      ...
      if qr_qwr_data["nbProjects"] > 0
        ...
        Request.find(:all,:include=>[:counter_log],:conditions => ["(work_package = ? or work_package = ?) and assigned_to = ? and is_stream = 'Yes' and stream_id = ? and counter_logs.validity = 1", WORKPACKAGE_QS_RMT_NAME[0], WORKPACKAGE_QS_RMT_NAME[1], qr.rmt_user, @stream.id]).each do |req|
        ...
        Request.find(:all,:include=>[:counter_log],:conditions => ["(work_package = ? or work_package = ?) and assigned_to = ? and is_stream = 'Yes' and stream_id = ? and counter_logs.validity = 1", WORKPACKAGE_SPIDERS_RMT_NAME[0], WORKPACKAGE_SPIDERS_RMT_NAME[1], qr.rmt_user, @stream.id]).each do |req|
