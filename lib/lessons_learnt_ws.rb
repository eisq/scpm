...
module LessonsLearntWs
  ...
  def self.generate_file_header(qwr, coc)
    return [[QWR_HEADER, qwr],[COC_HEADER, coc]]
  end
  ...
  def self.import(lessons, actions, assessments, file_name)
    ...
    template_type = LessonCollectTemplateType.find(:first, :conditions=>["name LIKE ?", APP_CONFIG['lesson_template_ws']])
    if template_type
